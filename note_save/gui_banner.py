...
from .configuration import BANNER_BACKGROUND_COLOR, PRIMARY_BACKGROUND_COLOR, DEFAULT_PADDING,\
    TEXT_COLOR, SAVE_FOLDER, DEFAULT_COLLECTION_NAME, VALID_FILE_NAME_PATTERN
...
class BannerFrame(Frame):
    ...
    def __init__(self, window, app):
        ...
        self.label = tk.Label(self.frame, text="Note Save",
                              bg=BANNER_BACKGROUND_COLOR, fg=TEXT_COLOR)
        self.label.pack()
    ...
    def event_open_collection(self, event=None):
        ...
        try:
            ...
            messagebox.showwarning(
                title="Warning", message="This collection cannot be opened because it has not "
    ...
    def event_button_add(self, event=None):
        ...
        # Title label
        label = tk.Label(self.new_collection_window, text="Enter a name for the collection",
                         bg=PRIMARY_BACKGROUND_COLOR, fg=TEXT_COLOR)
        ...
        self.new_collection_input.pack(
            fill=tk.BOTH, expand=True, padx=DEFAULT_PADDING, pady=DEFAULT_PADDING)
        self.new_collection_input.focus_set()
        self.new_collection_input.bind(
            "<Key>", self.event_typing_valid)
        self.new_collection_input.bind(
            "<Return>", self.event_enter_collection_name)
        # Ok button
        button_ok = tk.Button(self.new_collection_window,
                              text="Create", command=self.event_enter_collection_name)
        ...
        if self.new_collection_input is not None:
            ...
            if valid_file_name(collection_name) and len(collection_name) > 0:
                ...
                messagebox.showerror(
                    title="Error", message="The collection name must be a valid file name.\n"
                    f"{collection_name} does not respect {VALID_FILE_NAME_PATTERN}")
